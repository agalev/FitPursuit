...
export default function Toaster({ error }: { error: string }) {
	useEffect(() => {
		initTE({ Toast })
	}, [])
	...
	return (
		<div
			className='fixed bottom-5 right-5 z-50 flex justify-content pointer-events-auto hidden w-96 rounded-lg bg-danger-100 bg-clip-padding text-sm text-danger-700 shadow-lg shadow-black/5 data-[te-toast-show]:block data-[te-toast-hide]:hidden'
			id='static-example'
			role='alert'
			aria-live='assertive'
			aria-atomic='true'
